...
@Component({
  selector: 'app-rwf-dashboard',
  templateUrl: './rwf-dashboard.page.html',
  styleUrls: ['./rwf-dashboard.page.scss'],
})
export class RwfDashboardPage implements OnInit {

  public selectViewPoolManagerDivFlag: boolean = true;
  ...
  byPoolManagerChartDataSource = poolManagerChartData;
  byPoolManagerDataSource = poolManagerChartData;
  ...
  // Get RWF Dashboard data using API's
  getRwfDashboardData() {
    ...
    if (this.platform.is('cordova')) {
      ...
      if (this.network.checkNetWorkConnection()) {
        ...
        this.api.getRwfDropDownData(this.authToken)
          .then((response: any) => {

            // Dismiss the loader
            // this.dismissLoader();

            let rwfDropdownDataResponse = JSON.parse(response.data);
            console.log('RWF Dropdown Response -> ', rwfDropdownDataResponse)
            ...
            for (let i = 0; i < rwfDropdownDataResponse.buNames.length; i++) {
              let data = {
                "bu": "",
                "isSelected": false
              }
              data.bu = rwfDropdownDataResponse.buNames[i];
            ...
            for (let i = 0; i < rwfDropdownDataResponse.practiceNames.length; i++) {
              let data = {
                "practiceName": "",
                "isSelected": false
              }
              data.practiceName = rwfDropdownDataResponse.practiceNames[i];
              data.isSelected = true;
            ...
            for (let i = 0; i < rwfDropdownDataResponse.skillNames.length; i++) {
              let data = {
                "skillName": "",
                "isSelected": false
              }
              data.skillName = rwfDropdownDataResponse.skillNames[i];
              data.isSelected = true;
            ...
            for (let i = 0; i < rwfDropdownDataResponse.bandList.length; i++) {
              let data = {
                "bandName": "",
                "isSelected": false
              }
              data.bandName = rwfDropdownDataResponse.bandList[i];
              data.isSelected = true;
            ...
            this.poolAgeList.push({ "poolAge": "0-15 days", "isSelected": true, "value": "0" });
            this.poolAgeList.push({ "poolAge": "15-30 days", "isSelected": true, "value": "1" });
            this.poolAgeList.push({ "poolAge": "30-45 days", "isSelected": true, "value": "2" });
            this.poolAgeList.push({ "poolAge": "> 45 days", "isSelected": true, "value": "3" });
            ...
            this.selectYourViewList.push({ "buName": "Pool Age", "isSelected": true });
            this.selectYourViewList.push({ "buName": "Visa", "isSelected": true });
            this.selectYourViewList.push({ "buName": "Pool Manager", "isSelected": true });

            // API call to get the RWF Dashboard data
            this.getRwfDashboardDataFromAPI(this.authToken, buArray.toString(), practiceArray.toString(),
  ...
  // Select View Pool Manager check box change listener
  selectViewPoolManagerCheckBoxChangeEvent(data: any) {
    if (data == false) {
      this.selectYourViewList[6].isSelected = false;
      this.selectViewPoolManagerDivFlag = false;
    } else if (data == true) {
      this.selectYourViewList[6].isSelected = true;
      this.selectViewPoolManagerDivFlag = true;
    }
  }
  ...
  rwfDashboardSelectFiltersGoBtnClick() {
  ...
  // Generic method to get RWF Dashboard data
  getRwfDashboardDataFromAPI(authToken: string, buArray: string, practiceArray: string,
    skillArray: string, bandArray: string, poolArray: string) {

    // Show loader
    this.showLoader()

    this.api.getRwfDashboardData(authToken, buArray.toString(), practiceArray.toString(),
      skillArray.toString(), bandArray.toString(), poolArray.toString())
      .subscribe(response => {
        ...
        this.poolTileCount = topStripResponse.rwfPoolPipelineCount.rwfCount;
        this.pipeLineTileCount = topStripResponse.rwfPoolPipelineTotalCount.pipeline;
        this.visaResourcesTileCount = topStripResponse.rwfVisaCount.withVisa;
        this.majourSkillFamilyTileCount = topStripResponse.topFamilySkill['Not Rated'];
  ...
  plotMultipleGraphs(graphResponse: any) {
    ...
    for (let i = 0; i < graphResponse.bandCounts.length; i++) {
      this.byBandDataSource.data.push({
        label: graphResponse.bandCounts[i]._id,
        value: graphResponse.bandCounts[i].count
      })
    }

    // Plot By Band pie chart
    this.bySkillSetDataSource.data = [];
    this.bySkillSetDataSource.chart.caption = "BY SKILL SET";
    for (let key in graphResponse.skillCounts) {
      this.bySkillSetDataSource.data.push({
        label: key,
        value: graphResponse.skillCounts[key]
      })
    }
    ...
    for (let i = 0; i < graphResponse.poolAgeCounts.length; i++) {
      if (graphResponse.poolAgeCounts[i]._id == "0") {
        this.byPoolAgeDataSource.data.push({
          label: "0 - 15 days",
          value: graphResponse.poolAgeCounts[i].count
        })
      }
      if (graphResponse.poolAgeCounts[i]._id == "16") {
        this.byPoolAgeDataSource.data.push({
          label: "16-30 days",
          value: graphResponse.poolAgeCounts[i].count
        })
      }
      if (graphResponse.poolAgeCounts[i]._id == "31") {
        this.byPoolAgeDataSource.data.push({
          label: "31-45 days",
          value: graphResponse.poolAgeCounts[i].count
        })
      }
      if (graphResponse.poolAgeCounts[i]._id == "Infinity") {
        this.byPoolAgeDataSource.data.push({
          label: "> 45 days",
          value: graphResponse.poolAgeCounts[i].count
        })
    ...
    // Plot By Pool Manager donut chart
    this.byPoolManagerDataSource.data = [];
    this.byPoolManagerDataSource.chart.caption = "BY POOL MANAGER";
    for (let i = 0; i < graphResponse.pgmReporteesCounts.length; i++) {
      this.byPoolManagerDataSource.data.push({
        label: graphResponse.pgmReporteesCounts[i]._id,
        value: graphResponse.pgmReporteesCounts[i].count
      })
    }
  }
  ...
  // By Pool Manager more click button event
  poolManagerMoreBtnClick(event: any) {
    this.navCtrl.push('rwf-dashboard-more-details',
      {
        "pageTitle": "Associate Manager Details",
        "data": this.rwfResourceListResponseData
      })
  }
